...
function ClearHighScore(){
	if (confirm("Are you sure you want to clear your high score & current score?")) {
		...
		RestartGame('skip');
			$('#message').prepend("<span style='color: deepskyblue; font-size: 2em;' class='shadowLight'>Cleared High Score</span><br>");
	}
	else {
...
function Slide(direction){
	...
	function CrossoverMessage(){
		$('#message').prepend("<span style='color: deepskyblue; font-size: 1.5em;' class='shadowLight'>Crossover!</span><br>");
	}
...
function YayMoveButtons(y){
	...
	var buttonSideH = currentHitEdgeH.toUpperCase();
	var buttonSideW = hitEdgeW.toUpperCase();
	buttonSideH = 'slider' + buttonSideH;
	buttonSideW = 'slider' + buttonSideW;
	console.log(buttonSideH, buttonSideW, y);

	if(hitEdgeH != 'n' || hitEdgeW != 'n'){
		if(y == buttonSideW){
			BumpW(y);
			BounceMessage();
		}
		else if(y == buttonSideH){
			BumpH(y);
			BounceMessage();
		}
		else{
			console.log('received invalid input or no bump needed');
		}
		function BounceMessage(){
			$('#message').prepend("<span style='color: deepskyblue; font-size: 1.5em;' class='shadowLight'>Bounce-Back!</span><br>");
		}
